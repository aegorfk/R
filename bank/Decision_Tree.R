...
test_func <- function(rent) { if (rent <= -3.5) return("Низкая") else if (rent <= -2) return("Средняя") else if (rent >= -2) return("Высокая")}
bankruptcy$Рентабельность <- lapply(bankruptcy$Рентабельность.активов, test_func)
