...
# Подключаем JS и CSS  библиотеки
...
# Получаем доступ к онлайн-репозиториям
...
Для проверки полученной модели мы разделили все имеющиеся данные в соотношении 85:15. То есть тренировочными данными мы сделали 205 записей, мы выбрали их случайным образом из нашей выборки так, чтобы подвыборка была репрезентативная нашим данным.
...
sampind1 <- ind1[sample(1:nrow(ind1), 50, replace=FALSE),] # 85% банкротов
sampind0 <- ind0[sample(1:nrow(ind0), 155, replace=FALSE),] # 85% не банкротов
...
ind1 <- subset(bankruptcy, bankruptcy[,"Банкрот"]==1, select=ID: Банкрот)
ind0 <- subset(bankruptcy, bankruptcy[,"Банкрот"]==0, select=ID: Банкрот)
sampind1 <- ind1[sample(1:nrow(ind1), 50, replace=FALSE),]
sampind0 <- ind0[sample(1:nrow(ind0), 155, replace=FALSE),]
testing_data<-NULL
training_data<-NULL
...
legend("topright", c("Предсказанное", "Реальное", "Пересечения"), col=c("blue", "orange", "pink"), lwd=10)
```
