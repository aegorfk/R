...
```
Определив оптимальное количество кластеров, был применен метод k - средних, который разбил данные на 5 кластеров. Результат работы данного алгоритма можно продемонстрировать в виде матрицы диаграмм рассеивания:
```{r}
#lets see what we got
plot(training_data$values, col=party$cluster)
```
Рассмотрим распределение банкротов в полученных кластерах  на тренировочных данных:
```{r}
...
Совпадение предсказаний составило `r i`%. Отобразим распределение результатов байесовского классификатора на гистограмме. Морковным цветом отображается пересечение множеств "предсказанное" и "реальное" значения.  
